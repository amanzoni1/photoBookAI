...
class AIService:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.base_url = "https://cloud.lambdalabs.com/api/v1"
        self.api_key = config['LAMBDA_API_KEY']
        self.regions = config['LAMBDA_REGIONS']
        self.instance_types = config['LAMBDA_INSTANCE_TYPES']
        
        if isinstance(self.regions, str):
            self.regions = self.regions.split(',')
        if isinstance(self.instance_types, str):
            self.instance_types = self.instance_types.split(',')
    ...
    def launch_instance(self) -> LambdaInstance:
        """Launch a new Lambda instance with fallback options."""
        last_error = None
        ...
        for instance_type in self.instance_types:
            for region in self.regions:
                try:
                    ...
                    ssh_key_name = self.config['LAMBDA_SSH_KEY_NAME']
                    ...
                    if response.status_code == 400:
                        ...
                        if "insufficient-capacity" in error_message:
                            logger.warning(f"No capacity for {instance_type} in {region}, trying next region")
                            continue
                        elif "invalid-region" in error_message or "invalid-instance-type" in error_message:
                            logger.error(f"Invalid region or instance type: {error_message}")
                            break  
                        else:
                            logger.error(f"Error launching instance: {error_message}")
                    ...
                    logger.warning(f"Request timed out when launching {instance_type} in {region}")
                    last_error = "Request timed out"
                    continue
                except requests.exceptions.RequestException as req_err:
                    logger.warning(f"Request exception: {req_err}")
                    last_error = str(req_err)
                    continue
                except Exception as e:
                    logger.warning(f"Unexpected error with {instance_type} in {region}: {str(e)}")
                    last_error = e
                    continue

            logger.warning(f"No capacity or invalid instance type {instance_type} in all regions, trying next instance type")

        raise LambdaAPIException(f"Failed to launch instance with any type/region combination. Last error: {str(last_error)}")
    ...
    def train_model(self, model_id: int, user_id: int, training_config: Dict) -> str:
        """Run model training and return the local path to the weights file."""
        instance = None
        dataset_path = None
        local_weights = None
        ...
        try:
            ...
            remote_weights = f"{self.remote_workspace}/output/{model_name}/{model_name}.safetensors"
            local_weights = self.base_path / f"{model_name}.safetensors"

            instance.download_file_scp(remote_weights, str(local_weights))

            # Return the path to the local weights file
