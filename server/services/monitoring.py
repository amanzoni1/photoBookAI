...
from models import User, UserImage, TrainedModel, StorageLocation
from app import db
    ...
    def get_user_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            image_stats = db.session.query(
                db.func.count(UserImage.id),
                db.func.sum(UserImage.file_size)
            ).filter(UserImage.user_id == user_id).first()
            
            # Get model stats
            model_stats = db.session.query(
                db.func.count(TrainedModel.id)
            ).filter(TrainedModel.user_id == user_id).first()
            
            return {
                'total_images': image_stats[0] or 0,
