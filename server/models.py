...
JobStatusEnum = SAEnum(JobStatus, name="jobstatus", create_type=False)
...
class User(db.Model, UserMixin, TimestampMixin):
    ...
    credit_transactions = db.relationship(
        "CreditTransaction", backref="user", lazy=True
    )
...
class PhotoBook(db.Model, TimestampMixin):
    ...
    def theme_prompts(self) -> List[str]:
        """Get prompt strings for this theme from config."""
        theme_data = Config.PHOTOSHOOT_THEMES.get(self.theme_name, {})
        prompt_list = theme_data.get("prompts", [])
...
class GeneratedImage(db.Model, TimestampMixin):
    ...
    generation_job_id = db.Column(
        db.Integer, db.ForeignKey("generation_jobs.id"), nullable=True
    )
...
class GenerationJob(db.Model, TimestampMixin):
    ...
    generated_images = db.relationship(
        "GeneratedImage", back_populates="generation_job", lazy=True
    )
